defmodule Au4.AccountTest do
  use Au4.DataCase

  alias Au4.Account

  import Au4.AccountFixtures
  alias Au4.Account.{User, UserToken}
  ...
  describe "apply_user_email/3" do
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} = Account.apply_user_email(user, password, %{email: email})
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Account.deliver_user_update_email_instructions(user, "current@example.com", url)
        end)
